            ...
            while year_counter >= 0:
                try:
                    ...
                    for row in rows:
                        ...
                        cursor.execute('''INSERT INTO stock_data_PROTOTYPE (code, date, last_price, max_price, 
                                                   min_price, avg_price, percent_change,quantity, revenue_best_denars, total_revenue_denars) 
                                               VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)''', cleaned_data)
                        conn.commit()

                    year_counter -= 1
                except Exception as e:
                    print(f"Error during scraping year {year_counter}: {e}")
                    year_counter -= 1
